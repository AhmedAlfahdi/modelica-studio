...
test("the settings tab renders every section, with the solver's details in its block", async () => {
  ...
  const out = page(
    ...
    "const headings = Array.from(root.querySelectorAll('.setting-item-heading .setting-item-name')).map((h) => h.textContent);",
    ...
    "  const headingIndex = Array.from(root.querySelectorAll('.setting-item-heading'));",
    "  const inSection = at >= 0 && Array.from(root.querySelectorAll('.setting-item')).indexOf(item) > headingIndex.indexOf(headingIndex[at]);",
